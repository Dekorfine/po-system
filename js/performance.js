...
// === V3 兼容：性能统计优先用 ALL_PO_ORDERS（PO 全量含已完成），fallback 到旧 ORDERS ===
function _perfOrders() {
  return (typeof ALL_PO_ORDERS !== 'undefined' && ALL_PO_ORDERS.length > 0) ? ALL_PO_ORDERS : ORDERS;
}
...
function renderMonthlySummary() {
  ...
  // 1. 本月新增统计
  const monthlyOrders = _perfOrders().filter(o => isCreatedInMonth(o) || isInMonth(o.orderDate));
  ...
  const ordersPending = _perfOrders().filter(o => {
...
function exportMonthlySummary() {
  ...
  const monthlyOrders = _perfOrders().filter(o => isInMonth((o.createdAt || '').slice(0, 10)) || isInMonth(o.orderDate));
  const monthlyAfter = AFTERSALES.filter(a => isInMonth(a.createdDate));
...
function exportYearlySummary() {
  ...
  const monthly = months.map(m => {
    ...
    const o = _perfOrders().filter(x => (x.orderDate || '').startsWith(monthStr));
