...
function issAutoFetchProducts() {
  ...
  _issLastSrc = src;
  // V20260602:打开时用新抓取(已清洗+数量)重建已选产品 · 修"数量空/规格英文"
  if (t && _issFetched.length && _issFetched.some(p => p._checked)) {
    const fetchedSel = _issFetched.filter(p => p._checked).map(p => ({ spec: p.spec, qty: p.qty, image_url: p.image_url, sku: p.sku }));
    const sig = arr => JSON.stringify((arr||[]).map(p => [p.sku || '', p.spec || '', p.qty == null ? '' : p.qty, p.image_url || '']));
    if (sig(t.products) !== sig(fetchedSel)) _issPersist(tt => { tt.products = fetchedSel; });
  }
  issRenderFetchPanel();
